...
def _erase_gold(
    ...
    for yc, xc in pos:
        ...
        mask[y_start:y_end, x_start:x_end] |= circle_mask[
            mask_y_start:mask_y_end, mask_x_start:mask_x_end
        ]

    mask_num_pixels = mask.sum()
    ref = img[~mask]
    mean = np.mean(ref)
    std = np.std(ref)
    noise = rng.normal(mean, std, size=mask_num_pixels).astype(np.float16)
    img = img.astype(np.float16, copy=True)
    img[mask] = noise
    return img
...
def run_find_beads_3d(
    in_mics: str,  # path
    out_job_dir: str,  # path
    gold_nm: float = 10.0,
):
    job_dir = _job.JobDirectory.from_job_star(in_mics)
    assert isinstance(job_dir, _job.TomogramJobDirectory)
    _out_job_dir = _job.JobDirectory(Path(out_job_dir))
    ...
    for info in job_dir.iter_tomogram():
        ...
        if _out_job_dir.state() == _job.RelionJobState.ABORT_NOW:
            raise RuntimeError("Job aborted")

    df_tomo = starfile.read(job_dir.tomograms_star())
...
def run_erase_gold(
    ...
    tilt_save_dir = job_dir.path / "tilt_series"
    ...
    for _, row in df_tomo.iterrows():
        ...
        model_path = job_dir.relion_project_dir / str(row["TomoBeadModel"])
        edf_path = job_dir.relion_project_dir / str(row[ETOMO_FILE])
        ...
        for col in [MIC_NAME, MIC_ODD, MIC_EVEN]:
            if col in tilt_star_df.columns:
                ...
                fid = imodmodel.read(model_path)[["z", "y", "x"]].to_numpy(
                    dtype=np.float32
                )
                ...
                for mic_path in mic_paths:
                    ...
                    img_erased = _erase_gold(
                        img, fid_tr, rng, gold_nm / info.tomo_pixel_size
                    )
                    save_path = (
                        tilt_save_dir.relative_to(_out_job_dir.relion_project_dir)
                        / f"{mic_path.stem}_erased{mic_path.suffix}"
                    )
                    _to_update.append(str(save_path))
                    with mrcfile.open(
                        _out_job_dir.path / save_path, mode="w", permissive=True
                    ) as mrc_out:
