...
FN_BODIES = Annotated[
    str,
    {
        ...
        "tooltip": (
            ...
            "loop_<br>"
            "_rlnBodyMaskName<br>"
            "_rlnBodyRotateRelativeTo<br>"
            "_rlnBodySigmaAngles<br>"
            "_rlnBodySigmaOffset<br>"
            "large_body_mask.mrc 2 10 2<br>"
            "small_body_mask.mrc 1 10 2<br>"
            "head_body_mask.mrc 2 10 2</code>\n\n"
            "Where each data line represents a different body, and: "
            "<ul>"
            "<li>`_rlnBodyMaskName` contains the name of a soft-edged mask with values in "
            "[0,1] that define the body;</li> "
            "<li>`_rlnBodyRotateRelativeTo` defines relative to which other body this "
            "body rotates (first body is number 1);</li> "
            "<li>`_rlnBodySigmaAngles` and `_rlnBodySigmaOffset` are the standard "
            ...
            "Entries can be 'None' (without the ''s) or the name of a MRC map with an "
            "initial reference for that body. In case the entry is None, the reference "
            "will be taken from the density in the consensus refinement.\n"
            "Also note that larger bodies should be above smaller bodies in the STAR "
            "file. For more information, see the "
            "<a href='https://elifesciences.org/articles/36861'>multi-body paper</a>."
        ),
        "widget_type": PathDrop,
        "allowed_extensions": [".star"],
        "group": "I/O",
