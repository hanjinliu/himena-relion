...
class QJobScheduler(QtW.QWidget):
    def __init__(self, ui: MainWindow):
        ...
        # The "more" button
        btn = QtW.QPushButton("...")
        btn.setObjectName("SchedulerMoreActionButton")
        btn.setFixedWidth(24)
        menu = QtW.QMenu(btn)
        menu.setToolTipsVisible(True)
        btn.setMenu(menu)
        btn.setStyleSheet(
            "QPushButton#SchedulerMoreActionButton::menu-indicator { image: none; width: 0px; }"
        )
        action = menu.addAction("Preview job.star", self.preview_job_star)
        action.setToolTip(
            "Preview the job.star file that will be generated by these parameters"
        )
        action = menu.addAction("Copy job.star to clipboard", self.copy_job_star)
        action.setToolTip(
            "Copy the content of the job.star file that will be generated by these parameters to clipboard"
        )
        action = menu.addAction(
            "Paste parameters from job.star text", self.paste_from_job_star
        )
        action.setToolTip("Paste parameters from a job.star file in the clipboard.")
